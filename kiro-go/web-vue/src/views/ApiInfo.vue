...
<template>
  <div class="space-y-6 max-w-[1600px] mx-auto pb-10">
    ...
    <div class="flex items-center gap-2 px-1">
      <Globe class="w-4 h-4 text-[var(--primary)]" />
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <button
        ...
        :class="selectedIndex === i
          ? 'border-[var(--primary)] shadow-md shadow-[var(--primary)]/5 scale-[1.02]'
      ...
      >
        <div class="flex items-center gap-2.5 mb-2">
          ...
          <Check v-if="selectedIndex === i" class="w-3.5 h-3.5 text-[var(--primary)] ml-auto" />
        </div>
    ...
    <div class="space-y-4">
      <!-- Endpoint URL -->
      <div class="flex items-center gap-3 p-4 modern-card cursor-pointer group transition-colors hover:border-[var(--primary)]/30" @click="copy(base + selected.path)">
        <div :class="selected.color" class="w-2.5 h-2.5 rounded-full shrink-0 shadow-lg"></div>
        <code class="text-sm font-mono text-[var(--primary)] flex-1 truncate">{{ base }}{{ selected.path }}</code>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center bg-white/5 rounded-md p-0.5">
              <button
                ...
                :class="shell === s ? 'bg-[var(--primary)] text-white' : 'text-slate-500 hover:text-slate-300'"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="modern-card p-5 flex items-start gap-3">
        <Code class="w-5 h-5 text-[var(--primary)] shrink-0 mt-0.5" />
        <div>
